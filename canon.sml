...
struct
  ...
  fun splitlast([x]) = (nil,x)
    ...
    | splitlast _ = ErrorMsg.impossible "canon splitlast"
             ...
             of (most,T.JUMP(T.NAME lab, _)) =>
              ...
              | (most,T.CJUMP(opr,x,y,t,f)) =>
               (case (Symbol.look(table,t), Symbol.look(table,f))
                ...
                | _ => ErrorMsg.impossible "canon trace splitlast"
        end
    | trace _ = ErrorMsg.impossible "canon trace"
  ...
  and getnext(table,(b as (T.LABEL lab::_))::rest) =
     ...
     | getnext(_) = ErrorMsg.impossible "canon getnext"
